  ...
  return (
    <section className="py-12 lg:py-20 bg-gradient-to-br from-amber-50 via-orange-50 to-stone-50">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <h2 className="text-2xl sm:text-3xl lg:text-4xl font-bold text-center mb-8 lg:mb-12 text-amber-900">Why Choose Us</h2>
    ...
    <section className="py-12 lg:py-20 bg-white overflow-hidden">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-8 lg:mb-14">
          <span className="inline-block bg-amber-100 text-amber-700 text-xs font-semibold px-4 py-1.5 rounded-full mb-3 uppercase tracking-wider">Our Equipment</span>
          <h2 className="text-2xl sm:text-3xl lg:text-4xl font-bold text-amber-900">Professional Salon Tools</h2>
        ...
        {/* Sketchfab 3D Embed — Salon Chair */}
        <div className="mb-10 lg:mb-14 rounded-2xl overflow-hidden shadow-xl border border-amber-100">
          <div className="bg-gradient-to-r from-amber-800 to-orange-700 px-5 py-3 flex items-center gap-2">
            <span className="text-white text-sm font-semibold">🌐 3D Salon Interior — Interactive Model</span>
            <span className="ml-auto text-amber-200 text-xs">Drag to rotate • Scroll to zoom</span>
          </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-4">
          {tools.map((tool, i) => {
            ...
            return (
              ...
              >
                <div className={`bg-gradient-to-br ${tool.color} rounded-2xl p-4 text-center shadow-md hover:shadow-xl border border-amber-100 h-full`}>
...
function VideoSection() {
  ...
  const YT_ID = "Prf0cCC7EXQ";

  // Trigger autoplay when section scrolls into view (≥50% visible)
  useEffect(() => {
    ...
    >
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        >
          ...
          {/* YouTube iframe — loads & autoplays once visible */}
          {playing && (
            <iframe
        ...
        <p className="text-center text-stone-500 text-xs mt-4">
          Video plays automatically when visible • Click fullscreen for best experience
        </p>
      </div>
    </section>
  );
...
// ─── GALLERY CAROUSEL ──────────────────────────────────────────────────────────
function GallerySection({ images, current, onNext, onPrev, onDot, isAutoPlay, setIsAutoPlay }) {
  return (
    <section className="py-12 lg:py-20 bg-white">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-8 lg:mb-12">
          <span className="inline-block bg-amber-100 text-amber-700 text-xs font-semibold px-4 py-1.5 rounded-full mb-3 uppercase tracking-wider">Gallery</span>
          <h2 className="text-2xl sm:text-3xl lg:text-4xl font-bold text-amber-900">Our Work & Ambiance</h2>
        </div>
          ...
          {images.map((img, i) => (
            ...
            >
              ...
              <div className="absolute bottom-5 left-5 text-white">
                <p className="text-sm font-medium bg-black/40 backdrop-blur-sm px-3 py-1 rounded-full">{img.alt}</p>
              </div>
            </div>
          ))}

          {/* Arrows */}
          <button onClick={onPrev} className="absolute left-3 top-1/2 -translate-y-1/2 z-10 bg-white/20 hover:bg-white/40 backdrop-blur-sm text-white w-10 h-10 rounded-full flex items-center justify-center transition">
            <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 19l-7-7 7-7"/></svg>
          </button>
          <button onClick={onNext} className="absolute right-3 top-1/2 -translate-y-1/2 z-10 bg-white/20 hover:bg-white/40 backdrop-blur-sm text-white w-10 h-10 rounded-full flex items-center justify-center transition">
        ...
        <div className="flex items-center justify-center gap-3 mb-6">
          {images.map((_, i) => (
            <button
              key={i}
              onClick={() => onDot(i)}
              className={`rounded-full transition-all duration-300 ${i === current ? "w-6 h-2.5 bg-amber-600" : "w-2.5 h-2.5 bg-amber-200 hover:bg-amber-400"}`}
            />
          ))}
          <button
            onClick={() => setIsAutoPlay((p) => !p)}
            className={`ml-2 text-xs px-3 py-1 rounded-full border transition ${isAutoPlay ? "bg-amber-600 text-white border-amber-600" : "bg-white text-amber-700 border-amber-300"}`}
          >
        ...
        <div className="grid grid-cols-6 gap-2 lg:gap-3">
          {images.map((img, i) => (
            <button
              key={i}
              onClick={() => onDot(i)}
              className={`rounded-xl overflow-hidden aspect-square transition-all duration-300 ${i === current ? "ring-2 ring-amber-500 ring-offset-2 scale-105" : "opacity-60 hover:opacity-100"}`}
            >
              <img src={img.src} alt={img.alt} className="w-full h-full object-cover" />
            </button>
          ))}
...
function MapSection() {
  return (
    <section className="py-12 lg:py-20 bg-gradient-to-br from-amber-50 to-orange-50">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-8 lg:mb-12">
          <span className="inline-block bg-amber-100 text-amber-700 text-xs font-semibold px-4 py-1.5 rounded-full mb-3 uppercase tracking-wider">Find Us</span>
          <h2 className="text-2xl sm:text-3xl lg:text-4xl font-bold text-amber-900">Our Location</h2>
          <p className="text-stone-500 mt-3 text-sm lg:text-base">Visit us at our salon — centrally located for your convenience.</p>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 lg:gap-8 items-start">
          ...
          <div className="bg-white rounded-2xl shadow-md p-6 border border-amber-100 space-y-5">
            <div>
              <p className="text-xs text-amber-600 font-semibold uppercase tracking-wider mb-1">Address</p>
              <p className="text-stone-700 text-sm font-medium">Jl. Mengger Hilir No.162
<br />Sukapura, Kec. Dayeuhkolot, Kabupaten Bandung, Jawa Barat 40267</p>
            </div>
            <div>
              <p className="text-xs text-amber-600 font-semibold uppercase tracking-wider mb-1">Opening Hours</p>
              <div className="text-stone-700 text-sm space-y-1">
                <div className="flex justify-between"><span>Mon – Fri</span><span className="font-medium">09:00 – 20:00</span></div>
                <div className="flex justify-between"><span>Saturday</span><span className="font-medium">08:00 – 21:00</span></div>
                <div className="flex justify-between"><span>Sunday</span><span className="font-medium">10:00 – 18:00</span></div>
              </div>
            </div>
            <div>
              <p className="text-xs text-amber-600 font-semibold uppercase tracking-wider mb-1">Contact</p>
          ...
          {/* Map Embed */}
          <div className="lg:col-span-2 rounded-2xl overflow-hidden shadow-xl border border-amber-100" style={{ height: "380px" }}>
            <iframe
...
function TestimonialsSection() {
  const reviews = [
    { name: "Sarah Johnson", text: "Amazing experience! The staff is professional and the atmosphere is so relaxing." },
    { name: "Emily Davis", text: "Best salon in town! I always leave feeling beautiful and refreshed." },
    { name: "Jessica Miller", text: "Highly recommend! The quality of service is exceptional." },
  ];
  return (
    <section className="py-12 lg:py-20 bg-white">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <h2 className="text-2xl sm:text-3xl lg:text-4xl font-bold text-center mb-8 lg:mb-16 text-amber-900">What Our Clients Say</h2>
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-6 lg:gap-10">
          {reviews.map((item, i) => (
            <div key={i} className="bg-gradient-to-br from-amber-50 to-orange-50 p-6 lg:p-8 rounded-2xl shadow-md hover:shadow-xl transition border border-amber-200">
              <div className="mb-4 text-amber-500 text-lg">★★★★★</div>
              <p className="text-sm lg:text-base text-stone-700 mb-4 lg:mb-6 italic leading-relaxed">"{item.text}"</p>
              <p className="font-semibold text-amber-900 text-sm sm:text-base">{item.name}</p>
            </div>
          ))}
        </div>
      </div>
  ...
  return (
    <section className="py-12 lg:py-20 bg-gradient-to-r from-amber-600 via-orange-600 to-amber-700 text-white">
      <div className="max-w-4xl mx-auto text-center px-4 sm:px-6">
        <h2 className="text-2xl sm:text-3xl lg:text-4xl font-bold mb-4 lg:mb-6">Ready to Transform Your Look?</h2>
        <p className="text-sm sm:text-lg mb-6 lg:mb-8 text-amber-50">Book your appointment today and enjoy premium beauty treatments.</p>
        <a
          href="/booking"
